...
REF_DOME_INTENSITY = 1000.0
MARGIN = 1.1


def ref_pose_from_grasp(grasp_point, lo, hi, K, width, height, margin=MARGIN):
    centroid = 0.5 * (lo + hi)
    normal = grasp_point[:3, 0] / np.linalg.norm(grasp_point[:3, 0])
    up = np.array([0.0, 0.0, 1.0])
    half_w = 0.5 * float(np.dot(hi - lo, np.abs(np.cross(up, normal))))
    half_h = 0.5 * float(hi[2] - lo[2])
...
def render_one(app, rep, obj, K, width, height, runtime):
    ...
    geo = load_asset("/World/geo", str(obj.usd_path), ref_prim_path="/World")
    add_labels(geo, labels=[obj.meta["name"]], instance_name="instance")

    rng = local_bbox_range(geo)
    lo, hi = np.array(rng.GetMin()), np.array(rng.GetMax())
    ref_pose_cv = ref_pose_from_grasp(obj.grasp_point, lo, hi, K, width, height)

    setup_camera("ref_cam", "/World/ref_cam", width, height, K)
    set_prim_pose("/World/ref_cam", cv2opengl(ref_pose_cv))
    ...
    seg = a_seg.get_data()
    seg = seg["data"] if isinstance(seg, dict) else seg
    depth = np.where(seg != 0, depth, 0.0).astype(np.float32)
    return rgb, depth, ref_pose_cv


def main() -> None:
    runtime = load_config(sys.argv[1], sys.argv[4:])
    ...
    n = len(sorted((in_dir / "meta").glob("meta_*.yaml")))
    for idx in range(n):
