...
def _parser() -> argparse.ArgumentParser:
    ...
    arm = sub.add_parser(
        ...
        description=(
            ...
            "                  ONE config, no ingest loop: one from-scratch closed-ops train\n"
            "                  per size in --sizes, each on the base plus the first size-3\n"
            "                  pools, with every pool capped (retrain_loop.POOL_FRAME_CAP)\n\n"
