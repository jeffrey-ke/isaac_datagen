...
def extract_one(store, model_path: str, policy, tmp_dir: str) -> GraspableObject:
    ...
    p = f"{model_path}/v_0"
    prim = store.GetStage().GetPrimAtPath(p)
    assert prim.IsValid(), f"no v_0 transform node under {model_path}"
    rng = untransformed_bbox_range(prim)
    ...
    return GraspableObject(
        ...
        meta={"name": name, "class": cls,
              "store_prim": p[len(str(store.GetPath())) + 1:]},
...
def main() -> None:
    ...
    assert not dupes, f"duplicate SKU names among matched prims: {dupes}"
    ...
    with open(out_dir / "runtime.yaml", "w") as f:
