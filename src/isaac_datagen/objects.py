...
class OptFlowObject(SerializableSample):
    ...
    def visualize(self, *, depth_cmap="turbo", cam_scale=None, show_mesh=True,
        ...
        im = ax_d.imshow(np.ma.masked_equal(depth, 0.0), cmap=depth_cmap)
        ...
        extent_pts = [np.zeros(3), cam_C]
        if show_mesh:
            pts, faces, face_rgb = self._load_mesh()
    ...
    def _load_mesh(self):
        ...
        for prim in stage.Traverse():
            ...
            P = (l2w @ np.c_[P, np.ones(len(P))].T).T[:, :3]
            ...
            st_vals = np.asarray(st.ComputeFlattened(), dtype=np.float64) if st else None
            o = 0
            for c in counts:
                f = fidx[o:o + c]
        ...
        if tri_uvs:
            ...
            for prim in stage.Traverse():
                ...
                if sh and "UVTexture" in (sh.GetShaderId() or ""):
                    ...
                    break
            if tex is not None:
                uv = np.asarray(tri_uvs).mean(1)
                Ht, Wt = tex.shape[:2]
                u = np.clip(uv[:, 0], 0, 1) * (Wt - 1)
                v = (1.0 - np.clip(uv[:, 1], 0, 1)) * (Ht - 1)
...
class LoadedPallet:
    ...
    EPSILON = 0.002
    CONTACT_ROTATION = (0.0, 0.0, 180.0)
